...
static unsigned char* decoded_image_data = NULL;
...
int init_png_file(const char* input_file)
{
  ...
  unsigned char* encoded_image_data = NULL;
  ...
  if(error){
    print_error(stderr, "couldn't load file\n");
  ...
  if(error){
    print_error(stderr, "couldn't get png info\n");
    lodepng_state_cleanup(&state);
    return 1;
  }
  ...
  if(error || decoded_image_data == 0){
    print_error(stderr, "couldn't decode file %s\n", lodepng_error_text(error));
    lodepng_state_cleanup(&state);
...
int decrypt_png_file()
{
  ...
  int color_type = state.info_raw.colortype;
  ...
  if(bits_per_pixel == -1){
    print_error(stderr, "invalid bitdepth and color type\n");
    free(decoded_image_data);
    return 1;
  }
  ...
  if(bits_per_pixel % 8 == 0){
    ...
    for(int x = 0; x < (int) (sizeof(uint32_t) * 8); x++){
  ...
  if(number_of_bytes == 0){
    print_error(stderr, "couldn't read number_of_bytes\n");
    free(decoded_image_data);
    return 1;
  }
  ...
  if(block_area > (width * height) - (sizeof(uint32_t) * 8)){
    print_error(stderr, "invalid number of bytes for image size\n");
    free(decoded_image_data);
    return 1;
  }
  ...
  if(bits_per_pixel % 8 == 0){
    ...
    for(unsigned int x = 0; x < block_width; x++){
      ...
      for(unsigned int y = 0; y < block_height; y++){
        ...
        switch(color_type){
          case LCT_GREY:
            if(bitdepth == 16) pixel_index = 1;
            break;
          case LCT_RGB:
            if(bitdepth == 8) pixel_index = isaac_buffer[rand_count] % 3;
            else if(bitdepth == 16) pixel_index = 2 * (isaac_buffer[rand_count] % 3);
            break;
          case LCT_PALETTE:
            break;
          case LCT_GREY_ALPHA:
...
int encrypt_png_file()
{
  ...
  if(in_bytes == NULL){
    print_error(stderr, "read failed\n");
    lodepng_state_cleanup(&state);
    free(decoded_image_data);
    return 1;
  }
  ...
  if(height < 2 || width < 4){
    print_error(stderr, "image needs to be at least 2 pixels (height) by 4 pixels (width)\n");
    free(in_bytes);
    lodepng_state_cleanup(&state);
    free(decoded_image_data);
    return 1;
  ...
  if(block_area > (width * (height - 1))){
    print_error(stderr, "not enough space in image to encode data\nmax number_of_bytes to encrypt = %d\n", (width * (height - 1)) / 8);
    free(in_bytes);
    lodepng_state_cleanup(&state);
    free(decoded_image_data);
    return 1;
  ...
  if(bits_per_pixel % 8 == 0){
    int bytes_per_pixel = bits_per_pixel / 8;
    for(unsigned int x = 0; x < (sizeof(uint32_t) * 8); x++){
  ...
  if(error){
    print_error(stderr, "couldn't encode image data\n");
    return 1;
  }
  ...
  free(output_png);
  if(error){
    print_error(stderr, "couldn't save to file\n");
    return 1;
  }
